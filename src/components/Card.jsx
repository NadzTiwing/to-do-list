...
export default function Card() {
  ...
  const addItem = () => {
    ...
    items.unshift(newItem); // Marker A
    setItems((items) => [...items]); // Marker B
    setTotalTasks(items.length);

    /* 
      1. What is the difference between Marker A "items" and Marker B items* => [...item*]
      */
  ...
  /*
    * 2. What makes you decide to go with a single Card component?
    * 3. What is the advantage if any and;
    * 4. Disadvantage if any?
    */

  /*
    * 5. How can you improve the component rendering?
    * 6. Can you describe what is happening behind while you're typing on
    *   the input element? Marker C
  ...
  console.log(generateId()); // Marker C
      ...
      <div className="card-body">
        <div className="others">
          <p className="progress-txt">
            {totalFinished == items.length
              ? 'Awesome, you complete all of your tasks!'
              : `You finished ${totalFinished} out of ${totalTasks} tasks`}
          </p>
          <button className="add-btn btn" onClick={() => addItem()}>
            {' '}
            ADD TASK ( + ){' '}
          </button>
        </div>
            ...
            >
              {editItem !== item.id && (
                <div className="input-item my-2">
                  ...
                  <span className="px-2">
                    {item.done ? (
                      <strike className="strike">{item.name}</strike>
                    ) : (
                      item.name
                    )}
                  </span>
              ...
              <input
                type="text"
                className={editItem === item.id ? `form-control w-75` : `hide`}
                value={item.name}
              ...
              <div className="icons">
                <button
                  className={
                    editItem === item.id ? `btn btn-sm save-btn` : `hide`
                  }
                  onClick={() => saveTask(item.name)}
                >
                  SAVE
                </button>
                <button
                  className={isEditing ? `hide` : `btn btn-link`}
                  onClick={() => editTask(item.id)}
                >
                  <i className="pen-icon bi bi-pencil-fill"></i>
                </button>
                <button
                  className={isEditing ? `hide` : `btn btn-link`}
                  onClick={() => deleteTask(item.id)}
                >
                  <i className="trash-icon bi bi-x-lg"></i>
                </button>
              </div>
            </div>
          ))}
